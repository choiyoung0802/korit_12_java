...
/*
    배열의 출력
    엄밀히 말하면 저희는 배열을 출력한게 아니라 배열 내부의 element들을 출력해왔습니다.
    System.out.println(arr01); 과 같은 출력문을 작성했을 때, 주소값이 튀어나온다는
    사실을 확인할 수 있었습니다.

    근데 element의 값을 조정하거나, 더하거나 등의 추가적인 가공을 하기 위해서는 for문을
    사용해왔는데, 굳이 변형이 없고 배열 자체에 값이 들어갔는지만 확인하고 싶은데 매번
    main에다가 반복문을 작성을 하든지, 아니면 method를 정의해서 매번 호출해야하는지는 좀 의문이 듭니다.
...
//        Random random = new Random();   // 랜덬 클래스의 객체 생성
...
//        System.out.println(num2);
        // 이상의 경우에서 random.nextInt();는 int 범위 내에서 임의의 정수를 생성하는 메서드
        // scanner.nextInt();는 int 값을 콘솔에서 입력하는 메서드라는 점을 알 수 있다.
        // 즉, 같은 method명이면서도 어떤 클래스의 객체에서 호출하느냐에따라서 결과값이 다르네요.
...
//        System.out.println(Arrays.asList(numbers).toString()); // chaining method로 toString()이 있음을 확인
        Method01.call1();   // 객체명.메서드명(); 할 떄와 달리 이때는
...
//        System.out.println(Arrays.toString(Arrays.sort(nums)));
        ...
        /*
            이상의 경우에서 sort() 메서드에 오름차순에서는 argument가 하나 있는데,
            내림 차순할 때는 보니까 두 개 입니다.
            이상의 과정이 overloading이 이루어졌다는 점을 파악할 수 있으면 현재 수준으로는 만족스럽습니다.
            ...
            Integer[] 배열을 사용했다는 점도 특이사항 중 하나인데, 현재 수준으로는
