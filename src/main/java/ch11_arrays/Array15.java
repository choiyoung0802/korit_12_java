...
public class Array15 {
    public static void main(String[] args) {
        ...
        int round = 5; // 5게임으로 가정
        // 그리고 round를 이용하여 2차 배열을 만들어도 됨
//        int[][] lottoNumsSet = new int[round][6];
        ...
        for (int j = 0 ; j < round ; j++) {
            for (int i = 0; i < lottoNums.length; i++) {
                ...
                // 왜 temp에 넣었냐면 이 변수의 값이 배열 내부의 element 값과 다르면 집어넣는 과정을 추가하기 위해서입니다.
                /*
                    ...
                    일치하는게 있으면 -> 다시 난수 생성 후 temp에 값 대입하고 다시 확인하는 과정을 거쳐야만 합니다.
                    이하의 반복문은 3 번지라고 가정했을 때 2 번지까지의 element 값을 확인하는 부분입니다.
                 */
                for (int k = 0; k < i; k++) {
                    if (lottoNums[k] == temp) duplicate = true;
                }   // 어쨌든 0 번지부터 i-1 번지까지 전체 다 확인을할건데 한 번 true로 바뀌면 중복이 되었겠네요.
