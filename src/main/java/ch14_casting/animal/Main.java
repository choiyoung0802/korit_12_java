...
public class Main {
    public static void main(String[] args) {
        ...
        dog1.fetch(); // 결과값 : 강아지가 공을 물어옵니다.

        // 업캐스팅 # 1
        Animal animal1 = dog1;
        animal1.makeSound(); // 결과값 : 개가 짖습니다. - 재정의된 method가 호출됨.
//        animal1.fetch(); // 실행 불가능

        // 업캐스팅 # 2
        Animal animal2 = new Dog(); // A a = new B(); 형태. 시험에 자주 나옴.
        animal2.makeSound(); // 결과값 : 개가 짖습니다. - 재정의된 method가 호출됨.
//        animal2.fetch(); // 실행 불가능
    /*
        ...
        골치 안 아픈거 아니냐는 결론이 나올 수도 있음.
     */
        Animal animal0 = new Cat();
        animal0.makeSound(); // 결과값 : 고양이가 웁니다.
        animal1.makeSound(); // 결과값 : 개가 짖습니다.

        // 연산자 instanceof 사용 사래
        ...
        dog2.fetch();
        /*
            dog2로 새로운 객체명을 선ㅇ너하고 거기에 명시적 참조자료형 캣흐팅을 적용하여 Animal 클래스의
...
//        dog3.makeSound();
        /*
            이상의 경우를 확인했을 때 dog3.makeSound();로 가기 전에 이미 downcasting 부분에서 오류가
            발생했음을 확인할 수 있습니다.(77번 라인)
            즉, 애초에 animal3 를 생성할 때 Animal 클래스의 생성자를 호출해서 만들었기 때문에
            (A a = new A(); 이기 때문에)
            하위 클래스의 인스턴스로 다운캐스팅하는 것이 불가능합니다.
         */
        Animal animal4 = new Dog();
        // 와 같은 방식으로 선언된 클래스와 호출되는 생성자가 서로 달라야 합니다.(그리고 둘은 상속관계여야만 함)

        // 근데 문제는 77번 라인에서 빨간 줄 미리 띄워서 오류가 발생할 예정이라고 우리에게 알려주지 않는다는 점입니다.
