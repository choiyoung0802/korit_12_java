...
public class CentralControl {
    ...
    /*
        ...
        배열의 특성상 LastIndex+1 개의 방을 가지게 될겁니다. 그런데 5개의 방을
...
//        }
        ...
        if (emptyIndex == -1) {
            ...
            return; // 그래야 35번 라인이 실행 안돼서 추가 자체를 안 시킴.
        }
    ...
    public void powerOn() {
        ...
        for (int i = 0 ; i < deviceArray.length ; i++) {
            if (deviceArray[i] == null) {
                ...
                continue;   // return; 으로 마무리 했을 때와의 콘솔 창에서의 차이점을 확인하면 더 명확해집니다.
                /*
                    ...
                    그러면 6번지는 비어있고 7번지에 Power의 서브 클래스의
                    인스턴스가 존재하는 상황에서 6번지는 실행 안 되고
                    7번지는 실행되겠네요.
                 */
            }
            deviceArray[i].on();
    ...
    public void performSpecificMethod() {
        for (Power device : deviceArray) {
            if (device instanceof AirConditioner) {
                ...
                System.out.println("아직 지원되지 않는 전자기기 입니다.");
            }
        }
    }

    public void deleteDevice(int deviceDelete) {
        if (deviceDelete < 0 || deviceDelete >= deviceArray.length) return;
        if (deviceArray[deviceDelete] == null) return;
