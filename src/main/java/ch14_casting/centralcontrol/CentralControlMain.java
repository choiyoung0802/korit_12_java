...
public class CentralControlMain {
    public static void main(String[] args) {
        ...
        // 이하의 코드는 powers 배열은 만든 다음에 내부에 Power의 하위 클래스들의 객체를 집어넣었습니다.
        Power[] powers = {new AirConditioner(), new Computer(), new Mouse(), new LED(),
                          new Printer(), new Speaker()}; // 여기서 이루어졌습니다.(업캐스팅)

        /*
            근데 저희는 여기서 각 전자기기들의 객체를 생성하고 여기서 집어넣는 것까지 전체 다 실험할 예정이기 때문에
            powers를 직접 쓰지는 않을거고, Power가 interface이기 떄문에 객체 생성이 안 되는데 배열을 만들 수
            있는가를 검증하기 위해 코드를 남겨두겠습니다.
         */
