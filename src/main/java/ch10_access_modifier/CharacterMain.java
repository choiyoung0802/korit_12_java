...
/*
    ...
    2. method
        ...
        getExp(int amount) -> call2() 유형인데 경험치를 증가시키는 메서드(내부 전용)
            실행 예 :
            name 이(가) 경험치 amount을(를) 얻었습니다. 라고 출력되게.
 */
class Character {
    ...
    public Character(String name, int power, String skill) {
        ...
        this.exp = 0;       // 이상의 매개변수 생성자로 객체를 생성했을 때 체력 100 경험치 0으로 만든다는 의미
    }
    ...
    private void gainExp(int amount) { // method에 private을 처음 적용해봄.
        exp += amount;
        System.out.println(name + "(이)가 경험치를 " + amount + "을(를) 얻었습니다.");
    }

    // 두 가지 방식 중에 하나만 적용하면 됨.
    public void attack() {
        System.out.println(name + "(이)가 " + power + "으로 공격 !");
        ...
        System.out.println(name + "(이)가 경험치를 " + 10 + "을(를) 얻었습니다.");
    }
...
public class CharacterMain {
    public static void main(String[] args) {
        ...
        warrior.setSkill("세로 배기");
//        warrior.setExp(30);
        // 이상의 setExp의 경우 객체 생성하고 나서 경험치를 설정할 수 있는데
        // 여기에 1000000 같은 식으로 입력하면 부정 사용이 될 것 같긴 합니다.
        warrior.attack();
        warrior.heal();

//        warrior.getExp(1000); 경험치를 얻는 메서드는 attack() 메서드를
        // 호출함을 통해서만 얻을 수 있도록 하기 위해서는 접근 지정자가 private이 되어야겠네요.
