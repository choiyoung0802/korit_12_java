...
public class Method06StarWriting {
    ...
    public static String getStar(int row, int option) {
        ...
        if(option < 1 || option > 4) {
            result = "불가능한 메뉴 선택입니다. 1 ~ 4의 숫자만 입력해주세요.";
        } else {    // 이 부분이 실행된다면 어차피 option은 1, 2, 3, 4 중에 하나니까
            if (option == 1) {
                // 공백을 책임지는 바깥쪽 for문
                for (int i = 0 ; i < row + 1 ; i++) { // 여기 한계값이 i < row + 1 입니다.
                    // 별을 책임지는 안쪽 for문
                    for (int j = 0 ; j < i ; j++) { // i = 0, j = 0 일 때 별이 안 찍혀서
                        result += "*";
                    }
                    result += "\n";
                }
    ...
    public static void main(String[] args) {
        ...
        /*
            이상의 라인에 함축되어있는 의미를 파악할 필요가 있습니다.
